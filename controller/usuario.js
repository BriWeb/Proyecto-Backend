import { getUsuariosService, getUsuarioByIdService, agregarUsuarioService, editarUsuarioService, eliminarUsuarioService } from "../services/usuario.js"
...
export const getUsuarioByIdController = async (req, res) => {
    ...
    try {
        ...
        if (!usuario) {
            return res.status(404).send({ message: 'No se encontró ningún usuario con ID ' + req.params.id });
        }
        return res.status(200).send(usuario);
    } catch (error) {
        console.error(error);
        return res.status(500).send({ message: 'Error al obtener el usuario id: ' + id });
    }
}
...
export const agregarUsuarioController = async (req, res) => {
    ...
    try {
        ...
        //?? preguntar si el usuarioNuevo es un objeto vacio
        return res.status(201).send({ mensaje: 'Usuario agregado correctamente', Usuario: usuarioNuevo });
...
export const editarUsuarioController = async (req, res) => {
    ...
    try {
        ...
        if (!usuarioEditado) {
            return res.status(404).send({ mensaje: `No se encontró ningún usuario con ID ${id}` });
        }

        return res.status(200).send({ mensaje: 'El usuario ha sido editado correctamente', Usuario: usuarioEditado });
    } catch (error) {
        console.error(error);
